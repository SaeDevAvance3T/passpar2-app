...
public class ItineraryDetailActivity extends MenuActivity {
    ...
    private final String URL_START_COURSE = "https://2bet.fr/api/courses/start/";
    ...
    private AppCompatButton useAsCourseButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        useAsCourseButton = findViewById(R.id.use_as_course_button);

        useAsCourseButton.setOnClickListener(v -> useAsCourse());
    }
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            adapter = new EnterprisesItineraryDetails(ItineraryDetailActivity.this, enterpriseValues);
                            enterpriseList.setAdapter(adapter);
                            adapter.notifyDataSetChanged();

                        } catch (Exception e) {
                            e.printStackTrace();
                            Toast.makeText(ItineraryDetailActivity.this, "Erreur lors de la récupération des entreprises", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        Toast.makeText(ItineraryDetailActivity.this, "Erreur de connexion", Toast.LENGTH_SHORT).show();
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            e.printStackTrace();
                            Toast.makeText(ItineraryDetailActivity.this, "Erreur lors de la récupération des clients", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                new Response.ErrorListener() {
    ...
    public void useAsCourse() {
        ...
        String usedUrl = URL_START_COURSE + itineraryId;
        ...
        JsonObjectRequest requeteVolley = new JsonObjectRequest(
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject reponseJson) {
                        try {
                            Log.d("API_RESPONSE", "Réponse reçue : " + reponseJson.toString());

                            String status = reponseJson.getString("status");
                            Log.d("API_STATUS", "Status reçu : " + status);

                            if ("OK".equals(status)) {
                                JSONObject response = reponseJson.getJSONObject("response");
                                String courseId = response.getString("id"); // Directement une String

                                Log.d("courseId", "courseId " + courseId);

                                SharedPreferences sharedPreferences = getSharedPreferences("UserPrefs", Context.MODE_PRIVATE);
                                SharedPreferences.Editor editor = sharedPreferences.edit();
                                editor.putString("currentItineraryId", courseId); // Stocke l'ID de l'itinéraire
                                editor.apply();

                                // Création et lancement de l'intention avec l'ID récupéré
                                Intent intentionCourse = new Intent(ItineraryDetailActivity.this, Accueil_main.class);
                                intentionCourse.putExtra("itineraryId", courseId);
                                startActivity(intentionCourse);

                                Toast.makeText(getApplicationContext(), "Itinéraire lancé en tant que parcours !", Toast.LENGTH_SHORT).show();
                            } else {
                                Log.e("API_ERROR", "Statut non OK : " + status);
                                Toast.makeText(getApplicationContext(), "Erreur lors de la création du parcours", Toast.LENGTH_SHORT).show();
                            }
                        } catch (JSONException e) {
                            Log.e("JSON_ERROR", "Erreur JSON : " + e.getMessage());
